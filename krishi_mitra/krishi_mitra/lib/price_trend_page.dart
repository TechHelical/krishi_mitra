...
class PriceTrendPage extends StatefulWidget {
  ...
  const PriceTrendPage({super.key, required this.selectedState, required this.cropName});
...
class _PriceTrendPageState extends State<PriceTrendPage> {
  ...
  Future<List<Map<String, dynamic>>> predictPrice(String state, String crop, DateTime startDate, DateTime endDate) async {
    final apiURL = 'http://127.0.0.1:8080/predict';
  ...
  Future<List<Map<String, dynamic>>> fetchPastPrices(String state, String crop, DateTime month) async {
    final apiUrl = 'http://127.0.0.1:8080/pastPrices';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: _isLoading
                  ...
                  LineChartData(
                    ...
                    titlesData: FlTitlesData(
                      ...
                      leftTitles: AxisTitles(
                        sideTitles: SideTitles(
                          showTitles: true,
                          getTitlesWidget: (value, meta) {
                            return Text('₹${oCcy.format(value.toInt())}', style: const TextStyle(fontSize: 10, color: Colors.brown));
                          },
                          reservedSize: 40,
                    ...
                    : const Center(child: Text("No price data available to display.", style: TextStyle(fontSize: 16, color: Colors.grey))),
              ),
              const SizedBox(height: 15),

              // Expandable Past Prices Section
              GestureDetector(
                onTap: () {
                  setState(() {
                    showPastPrices = !showPastPrices;
                  });
                },
                child: Card(
                  color: Colors.yellow[200],
                  child: ListTile(
                    title: const Text("Past 6 Months Prices", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    trailing: Icon(showPastPrices ? Icons.expand_less : Icons.expand_more),
                  ),
                ),
              ),
              if (showPastPrices) ...[
                  ...
                  itemBuilder: (context, index) {
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: Text("₹${oCcy.format(pastPredictions[index]["price"])}", style: const TextStyle(fontSize: 16)),
                      ),
                    );
                  },
                ),
              ...
              const SizedBox(height: 15),
              const Text("Future Price Predictions:", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.brown)),
              _isLoading // Loading indicator for future prices
                ...
                itemBuilder: (context, index) {
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: Text("₹${oCcy.format(monthlyPredictions[index]["price"])}", style: const TextStyle(fontSize: 16)),
                    ),
                  );
                },
              ),
